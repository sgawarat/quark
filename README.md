...
- 管理者権限のアプリケーション上で機能しない：
  - このアプリケーションを管理者権限で起動することによって問題を回避できます。
  - （Windowsの仕様により、標準権限では管理者権限のキー入力を取得できません。）
- キーが押したままになる：
...
- 画面ロック時にキーが押したままになる不具合に対して緩和策を実装しました。

### 2026.08

- actionによる修飾キー状態の優先度をマスクを用いて制御する方法を追加しました。
  - 併せて、同等の機能を持つpseudo modsは廃止されました。
- リセット機能を追加しました。
  - リセットすると、内部状態を初期化してキーをすべて離した扱いにします。
  - アプリではシステムトレイアイコンのコンテキストメニューから呼び出せます。
- システムの有効・無効が判別できるようにアイコンを設定しました。
- tmk_coreの各機能の有効・無効をユーザー側で選択できるようになりました。
  - これにより、actionmapsの代わりにkeymapsを使ったり、NKRO機能を有効化したりできるようになります。
